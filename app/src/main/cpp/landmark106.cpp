...
MTCNN::MTCNN(const string &model_path) {
    std::vector<std::string> param_files = { model_path + "/landmark.param"};
    std::vector<std::string> bin_files = {model_path + "/landmark.bin"};
    mtcnn_net.load_param(param_files[0].data());
    mtcnn_net.load_model(bin_files[0].data());
}
...
void MTCNN::Detect(ncnn::Mat &img_, ncnn::Mat &output) {
    // Create extractor for network forwarding
    ncnn::Extractor ex = mtcnn_net.create_extractor();
    // Set multi-thread processing
    ...
    //Facial landmarks(106 points) inference through network forwarding
    ex.input("data", img);
...
//MTCNN::MTCNN(){}
MTCNNBox::MTCNNBox(const string &model_path) {
...
MTCNNBox::MTCNNBox(const std::vector<std::string> param_files, const std::vector<std::string> bin_files){
    Pnet.load_param(param_files[0].data());
    Pnet.load_model(bin_files[0].data());
    Rnet.load_param(param_files[1].data());
    Rnet.load_model(bin_files[1].data());
...
void MTCNNBox::SetMinFace(int minSize){
    minsize = minSize;
}

void MTCNNBox::SetNumThreads(int numThreads){
    num_threads = numThreads;
}

void MTCNNBox::SetTimeCount(int timeCount) {
    count = timeCount;
}


void MTCNNBox::generateBbox(ncnn::Mat score, ncnn::Mat location, std::vector<Bbox>& boundingBox_, float scale){
    const int stride = 2;
...
void MTCNNBox::nmsTwoBoxs(vector<Bbox>& boundingBox_, vector<Bbox>& previousBox_, const float overlap_threshold, string modelname)
{
...
void MTCNNBox::nms(std::vector<Bbox> &boundingBox_, const float overlap_threshold, string modelname){
    if(boundingBox_.empty()){
        return;
    }
    sort(boundingBox_.begin(), boundingBox_.end(), cmpScore);
...
void MTCNNBox::refine(vector<Bbox> &vecBbox, const int &height, const int &width, bool square){
    if(vecBbox.empty()){
...
void MTCNNBox::extractMaxFace(vector<Bbox>& boundingBox_)
{
    if (boundingBox_.empty()) {
        return;
    }
...
void MTCNNBox::PNet(float scale)
{
    //first stage
...
void MTCNNBox::detect(ncnn::Mat& img_, std::vector<Bbox>& finalBbox_){
    img = img_;
    img_w = img.w;
    img_h = img.h;
    img.substract_mean_normalize(mean_vals, norm_vals);
...
void MTCNNBox::detectMaxFace(ncnn::Mat& img_, std::vector<Bbox>& finalBbox) {
